#include "barbar-diagram.h"

struct _BarBarDiagram {
...
static GParamSpec *diagram_props[NUM_PROPERTIES] = {
    NULL,
};

G_DEFINE_TYPE(BarBarDiagram, g_barbar_diagram, GTK_TYPE_WIDGET)

static void g_barbar_diagram_set_value(BarBarDiagram *diagram,
                                       const char *path) {
  g_return_if_fail(BARBAR_IS_DIAGRAM(diagram));

  // g_free(bar->path);
  // bar->path = g_strdup(path);

  g_object_notify_by_pspec(G_OBJECT(diagram), diagram_props[PROP_VALUE]);
}

static void g_barbar_diagram_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec) {
  BarBarDiagram *diagram = BARBAR_DIAGRAM(object);
...
static void g_barbar_diagram_get_property(GObject *object, guint property_id,
...
void g_barbar_diagram_measure(GtkWidget *widget, GtkOrientation orientation,
                              int for_size, int *minimum, int *natural,
                              int *minimum_baseline, int *natural_baseline) {}

void g_barbar_diagram_snapshot(GtkWidget *widget, GtkSnapshot *snapshot) {
...
static void g_barbar_diagram_class_init(BarBarDiagramClass *class) {
  GObjectClass *gobject_class = G_OBJECT_CLASS(class);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS(class);

  gobject_class->set_property = g_barbar_diagram_set_property;
  gobject_class->get_property = g_barbar_diagram_get_property;

  widget_class->measure = g_barbar_diagram_measure;
  widget_class->snapshot = g_barbar_diagram_snapshot;
  diagram_props[PROP_VALUE] =
...
static void g_barbar_diagram_init(BarBarDiagram *self) {
  self->cur_value = 0.0;
  self->min_value = 0.0;
  self->max_value = 1.0;
}
