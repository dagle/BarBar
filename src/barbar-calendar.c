#include "barbar-calendar.h"
#include <gtk/gtk.h>
#include <stdio.h>

#include <string.h>

struct _BarBarCalendar {
...
typedef struct _BarBarCalendarClass BarBarCalendarClass;

struct _BarBarCalendarClass {
  GtkWidgetClass parent_class;

  void (*start)(BarBarCalendar *cal);
};

G_DEFINE_TYPE(BarBarCalendar, g_barbar_calendar, GTK_TYPE_WIDGET)
...
//
// static void add_cancellable(GtkPathBar *path_bar, GCancellable *cancellable)
// {
//   g_assert(g_list_find(path_bar->cancellables, cancellable) == NULL);
//   path_bar->cancellables = g_list_prepend(path_bar->cancellables,
//   cancellable);
// }
//
// static void drop_node_for_cancellable(GtkPathBar *path_bar,
...
// }
//
// static void cancel_cancellable(GtkPathBar *path_bar,
//                                GCancellable *cancellable) {
//   drop_node_for_cancellable(path_bar, cancellable);
//   g_cancellable_cancel(cancellable);
// }
//
// static void cancellable_async_done(GtkPathBar *path_bar,
//                                    GCancellable *cancellable) {
//   drop_node_for_cancellable(path_bar, cancellable);
...
static void g_barbar_calendar_init(BarBarCalendar *cal) {
  // cal->label = gtk_label_new("apa");
  cal->label = gtk_scrolled_window_new();

  gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(cal->label),
                                 GTK_POLICY_EXTERNAL, GTK_POLICY_NEVER);
  gtk_widget_set_hexpand(cal->label, TRUE);
  gtk_widget_set_parent(cal->label, GTK_WIDGET(cal));

  cal->buttons_box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
  gtk_widget_add_css_class(cal->buttons_box, "linked");
  gtk_scrolled_window_set_child(GTK_SCROLLED_WINDOW(cal->label),
                                cal->buttons_box);
  // const char *home;
  //
...
static void g_barbar_calendar_class_init(BarBarCalendarClass *path_bar_class) {
  ...
  //       I_("path-clicked"), G_OBJECT_CLASS_TYPE(gobject_class),
  //       G_SIGNAL_RUN_FIRST, G_STRUCT_OFFSET(GtkPathBarClass, path_clicked),
  //       NULL, NULL, _gtk_marshal_VOID__OBJECT_OBJECT_BOOLEAN, G_TYPE_NONE, 3,
...
static void gtk_path_bar_finalize(GObject *object) {
  G_OBJECT_CLASS(g_barbar_calendar_parent_class)->finalize(object);
...
static void gtk_path_bar_clear_buttons(BarBarCalendar *cal) {
  // GtkWidget *w;
  //
...
// static void update_visibility_up_to_next_root(GtkPathBar *path_bar,
...
//     return;
//
//   path_bar = GTK_PATH_BAR(gtk_widget_get_ancestor(button,
//   GTK_TYPE_PATH_BAR));
//
//   button_list = g_list_find(path_bar->button_list, button_data);
//   g_assert(button_list != NULL);
//
//   g_signal_handlers_block_by_func(button, G_CALLBACK(button_clicked_cb),
//   data); gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(button), TRUE);
//   g_signal_handlers_unblock_by_func(button, G_CALLBACK(button_clicked_cb),
//                                     data);
//
...
// };
//
// static void set_button_image_get_info_cb(GObject *source, GAsyncResult
// *result,
//                                          gpointer user_data) {
...
// }
//
// static void set_button_image(GtkPathBar *path_bar, ButtonData *button_data) {
...
//       root_icon = path_bar->root_icon =
//           g_object_ref(g_themed_icon_new("drive-harddisk-symbolic"));
...
//     g_file_query_info_async(path_bar->home_file, "standard::symbolic-icon",
//                             G_FILE_QUERY_INFO_NONE, G_PRIORITY_DEFAULT,
//                             button_data->cancellable,
//                             set_button_image_get_info_cb, data);
...
// }
//
// static const char *get_dir_name(ButtonData *button_data) {
...
// }
//
// static gboolean file_is_recent_uri(GFile *file) {
//   GFile *recent;
//   gboolean same;
//
...
// }
//
// static ButtonType find_button_type(GtkPathBar *path_bar, GFile *file,
//                                    GFile *root_file) {
//   if (root_file != NULL && g_file_equal(file, root_file))
//     return ROOT_BUTTON;
//   if (path_bar->home_file != NULL && g_file_equal(file, path_bar->home_file))
//     return HOME_BUTTON;
...
//                                          const char *dir_name, GFile *file,
//                                          GMount *mount, GFile *root_file,
//                                          gboolean current_dir,
...
//                    G_CALLBACK(button_clicked_cb), button_data);
...
//   gtk_widget_add_controller(button_data->button,
//   GTK_EVENT_CONTROLLER(source));
//
//   return button_data;
// }
//
// static gboolean gtk_path_bar_check_parent_path(GtkPathBar *path_bar,
//                                                GFile *file) {
//   GList *list;
...
//       BUTTON_DATA(list->data),
//                                             (list == current_path) ? TRUE
//                                                                    : FALSE);
//     }
//
//     if (!gtk_widget_get_child_visible(
//             BUTTON_DATA(current_path->data)->button)) {
...
// };
//
// static void gtk_path_bar_set_file_finish(struct SetFileInfo *info,
//                                          gboolean result) {
//   if (result) {
//     GList *l;
//
...
//
//       gtk_box_remove(GTK_BOX(info->path_bar->buttons_box),
//       button_data->button);
//     }
//
...
//
//   button_data = make_directory_button(
//       file_info->path_bar, display_name, file_info->file, file_info->mount,
//       file_info->root_file, file_info->first_directory, is_hidden);
//   g_clear_object(&file_info->file);
//
//   file_info->new_buttons = g_list_prepend(file_info->new_buttons,
//   button_data);
//
//   if (BUTTON_IS_FAKE_ROOT(button_data))
//     file_info->fake_root = file_info->new_buttons;
//
//   /* We have assigned the info for the innermost button, i.e. the deepest
//    * directory. Now, go on to fetch the info for this directory's parent.
//    */
//
...
//                           gtk_path_bar_get_info_callback, file_info);
//   add_cancellable(file_info->path_bar, file_info->cancellable);
// }
//
// static void gtk_path_bar_get_mount_callback(GObject *source,
//                                             GAsyncResult *result,
//                                             gpointer data) {
//   GFile *file = G_FILE(source);
//   struct SetFileInfo *file_info = data;
...
// }
//
// void _gtk_path_bar_set_file(GtkPathBar *path_bar, GFile *file,
//                             gboolean keep_trail) {
...
//                             G_FILE_QUERY_INFO_NONE, G_PRIORITY_DEFAULT,
//                             info->cancellable,
//                             gtk_path_bar_get_info_callback, info);
//   } else {
//     g_file_find_enclosing_mount_async(info->file, G_PRIORITY_DEFAULT,
//                                       info->cancellable,
//                                       gtk_path_bar_get_mount_callback, info);
//   }
...
//  *
//  * If the selected button in the pathbar is not the furthest button “up” (in
//  the
...
//  * @path_bar: a `GtkPathBar`
//  *
//  * If the selected button in the pathbar is not the furthest button “down”
//  (in *the leaf direction), act as if the user clicked on the next button
//  down.
//  **/
