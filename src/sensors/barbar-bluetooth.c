...
static void g_barbar_bluetooth_class_init(BarBarGameModeClass *class) {
  ...
  //     "enabled", NULL, NULL, FALSE, G_PARAM_READABLE |
...
static void g_barbar_game_mode_init(BarBarBluetooth *mode) {}
...
static void update_failed(GObject *object, GAsyncResult *res, gpointer data) {
  ...
  // GVariant *ret = g_dbus_proxy_call_finish(G_DBUS_PROXY(object), res,
  // &error);
  //
  // if (error || !ret) {
...
static void get_failed_units(BarBarBluetooth *units) {
  // g_dbus_proxy_call(
  //     units->proxy, "Get",
...
static void get_default_value(BarBarBluetooth *units) {
