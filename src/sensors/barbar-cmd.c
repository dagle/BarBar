...
void g_barbar_cmd_set_cmd(BarBarCmd *self, const GValue *value) {
  g_return_if_fail(BARBAR_IS_CMD(self));

  g_strfreev(self->cmd);
  self->cmd = g_value_dup_boxed(value);
...
static void g_barbar_cmd_set_property(GObject *object, guint property_id,
  ...
  switch (property_id) {
  case CMD_PROP_CMD:
    g_barbar_cmd_set_cmd(cmd, value);
    break;
...
static void g_barbar_cmd_start(BarBarCmd *cmd, gpointer data);
...
static void g_barbar_cmd_class_init(BarBarCmdClass *class) {
  ...
  gobject_class->constructed = g_barbar_cmd_constructed;
