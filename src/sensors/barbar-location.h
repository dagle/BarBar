...
static void gclue_service_agent_agent_iface_init(GClueAgentIface *iface);
static void gclue_service_agent_constructed(GObject *object);
...
G_DEFINE_TYPE_WITH_CODE(
    GClueServiceAgent, gclue_service_agent, GCLUE_TYPE_AGENT_SKELETON,
    G_IMPLEMENT_INTERFACE(GCLUE_TYPE_AGENT,
                          gclue_service_agent_agent_iface_init)
        G_ADD_PRIVATE(GClueServiceAgent))

enum { PROP_0, PROP_CONNECTION, LAST_PROP };

static GParamSpec *gParamSpecs[LAST_PROP];

static void gclue_service_agent_finalize(GObject *object) {
...
static void gclue_service_agent_get_property(GObject *object, guint prop_id,
...
static void gclue_service_agent_set_property(GObject *object, guint prop_id,
                                             const GValue *value,
                                             GParamSpec *pspec) {
  GClueServiceAgent *agent = GCLUE_SERVICE_AGENT(object);
...
static void gclue_service_agent_class_init(GClueServiceAgentClass *klass) {
...
static void gclue_service_agent_init(GClueServiceAgent *agent) {
  agent->priv = gclue_service_agent_get_instance_private(agent);
}

static void on_add_agent_ready(GObject *source_object, GAsyncResult *res,
                               gpointer user_data) {
  GError *error = NULL;
...
static void print_in_use_info(GDBusProxy *manager_proxy) {
  GVariant *variant;
...
static void on_manager_proxy_ready(GObject *source_object, GAsyncResult *res,
                                   gpointer user_data) {
  GClueServiceAgent *agent;
  g_autoptr(GError) error = NULL;

  agent = GCLUE_SERVICE_AGENT(user_data);
  agent->priv->manager_proxy = g_dbus_proxy_new_for_bus_finish(res, &error);
  if (agent->priv->manager_proxy == NULL) {
    g_critical("Failed to create proxy to %s: %s", MANAGER_PATH,
               error->message);
    return;
  }

  g_dbus_proxy_call(agent->priv->manager_proxy, "AddAgent",
                    g_variant_new("(s)", "geoclue-demo-agent"),
                    G_DBUS_CALL_FLAGS_NONE, -1, NULL, on_add_agent_ready, NULL);
  print_in_use_info(agent->priv->manager_proxy);
  g_signal_connect(agent->priv->manager_proxy, "g-properties-changed",
                   G_CALLBACK(on_manager_props_changed), NULL);
}
...
static void on_name_appeared(GDBusConnection *connection, const gchar *name,
  ...
  g_dbus_proxy_new_for_bus(G_BUS_TYPE_SYSTEM, G_DBUS_PROXY_FLAGS_NONE, NULL,
                           SERVICE, MANAGER_PATH, MANAGER_INTERFACE, NULL,
                           on_manager_proxy_ready, user_data);
}
...
static void gclue_service_agent_constructed(GObject *object) {
  GClueServiceAgent *agent = GCLUE_SERVICE_AGENT(object);
  G_OBJECT_CLASS(g_barbar_location_parent_class)->constructed(object);

  GError *error = NULL;
  if (!g_dbus_interface_skeleton_export(G_DBUS_INTERFACE_SKELETON(agent),
                                        agent->priv->connection, AGENT_PATH,
                                        &error)) {
    return;
  }
  gclue_agent_set_max_accuracy_level(GCLUE_AGENT(agent),
                                     GCLUE_ACCURACY_LEVEL_EXACT);
  g_bus_watch_name(G_BUS_TYPE_SYSTEM, SERVICE, G_BUS_NAME_WATCHER_FLAGS_NONE,
                   on_name_appeared, on_name_vanished, agent, NULL);
}
...
static void notification_data_free(NotificationData *data) {
...
static void on_notify_action(NotifyNotification *notification, char *action,
                             gpointer user_data) {
...
static void on_notify_closed(NotifyNotification *notification,
                             gpointer user_data) {
  NotificationData *data = (NotificationData *)user_data;
  ...
  gclue_agent_complete_authorize_app(data->agent, data->invocation,
                                     data->authorized, data->accuracy_level);
  notification_data_free(data);
}

static gboolean gclue_service_agent_handle_authorize_app(
    GClueAgent *agent, GDBusMethodInvocation *invocation,
    const char *desktop_id, GClueAccuracyLevel accuracy_level) {
  NotifyNotification *notification;
  ...
  notification =
      notify_notification_new(_("Geolocation"), msg, "dialog-question");
  ...
  notify_notification_add_action(notification, ACTION_YES, _("Yes"),
                                 on_notify_action, data, NULL);
  notify_notification_add_action(notification, ACTION_NO, _("No"),
                                 on_notify_action, data, NULL);
  g_signal_connect(notification, "closed", G_CALLBACK(on_notify_closed), data);
...
GClueServiceAgent *gclue_service_agent_new(GDBusConnection *connection) {
  return g_object_new(GCLUE_TYPE_SERVICE_AGENT, "connection", connection, NULL);
}
